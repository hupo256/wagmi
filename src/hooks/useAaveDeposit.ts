    ...
    for (let attempt = 1; attempt <= maxRetries; attempt++) {
      try {
        ...
        const approveUserOp = await smartAccount.buildUserOp(
          [approveTx],
          usePaymaster ? { paymasterServiceData: { mode: "SPONSORED" } } : undefined,
        );
        ...
        console.error(`Deposit attempt ${attempt} failed:`, err);
        if (attempt === maxRetries) {
          setDepositStatus(`Deposit failed after ${maxRetries} attempts: ${err.message || String(err)}`);
        } else {
          setDepositStatus(`Attempt ${attempt} failed, retrying... (${err.message || "Unknown error"})`);
